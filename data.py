...
def work(model_path, w, env_id, states):
    Actions = []
    Rewards =[]
    next_states = []
    States = []
...
def workers(model_path, States, w, env_id, n_cpu=6):

    seqences = np.array_split(States, n_cpu)
    w_list =  [w for _ in range(n_cpu)]
    env_list = [env_id for _ in range(n_cpu)]
    models_list = [model_path for _ in range(n_cpu)]
    partial_func = partial(work, w=w, env_id=env_id, model_path=model_path)
    with concurrent.futures.ProcessPoolExecutor(max_workers=None) as executor:        
        ...
        future_to_samples = {executor.submit(partial_func, states): states for states in seqences}
    S, A, R, NS = [], [], [], []
...
if __name__ == '__main__':
    ...
    out = workers(path, States, w, env_id = 'gym_seir:seir-v0', n_cpu=8)
    print(out)