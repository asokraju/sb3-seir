...
def init():
    ...
    sc = ax.scatter(xx, yy, zz, s=3*df['costs'], c=ACTIONS, marker='o', cmap=cmap, alpha=0.6)
    ax.set_xlabel('S')
    ax.set_ylabel('E')
    ax.set_zlabel('I')
    x = np.linspace(0., 1e5,1000)
...
anim = animation.FuncAnimation(fig, animate, init_func=init,
                               frames=3600, interval=1, blit=True)
# Save
anim.save('basic_animation_rewards2.mp4', fps=200, extra_args=['-vcodec', 'libx264'])
# 
...
# writer = FFMpegWriter(fps=100, bitrate=5000, metadata=metadata)

# anim.save('basic_animation_rewards1.mp4',  writer=writer)
